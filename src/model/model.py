...
class MMEBModel(nn.Module):
    ...
    def load(cls, model_args: ModelArguments, is_trainable=True, **kwargs):
        ...
        if getattr(model_args, "checkpoint_path", None):
            ...
            if os.path.isfile(metadata_path):
                with open(metadata_path, "r", encoding="utf-8") as handle:
                    hybrid_metadata = json.load(handle)
        ...
        if model_args.lora:
            ...
            if (
                ...
                if (
                    hybrid_metadata is not None
                    and hybrid_metadata.get("adapter_scope") == "encoder.model"
                ):
                    # Locally trained Stage 1 checkpoints wrap only the language
                    # model and keep the vision tower in the outer Qwen-VL model.
                    lora_model = PeftModel.from_pretrained(
                        base_model.model,
                        model_name_or_path,
                        config=lora_config,
                    ...
                    # Released VLM2Vec adapters are exported against the full
            ...
            if hybrid_metadata is not None:
                ...
                full_state_path = os.path.join(
                    model_name_or_path,
                    hybrid_metadata.get("full_state_file", "model.safetensors"),
                )
                if not os.path.isfile(full_state_path):
                    raise FileNotFoundError(
                        f"Hybrid checkpoint is missing full state: {full_state_path}"
                    )
                incompatible = encoder.load_state_dict(
                    load_file(full_state_path, device="cpu"),
                    strict=False,
                )
                allowed_missing = set(
                    hybrid_metadata.get("allowed_missing_keys", [])
                )
                missing = [
                ...
                if missing or incompatible.unexpected_keys:
                    raise RuntimeError(
                        "Hybrid checkpoint state mismatch: "
                        f"missing={missing}, "
                ...
                if not is_trainable:
                    ...
                    if nested_model is not None and hasattr(
                        nested_model, "merge_and_unload"
                    ):
                        encoder.model = nested_model.merge_and_unload()
