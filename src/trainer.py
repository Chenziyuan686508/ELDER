...
def _save_nested_peft_adapter(encoder, output_dir: str) -> None:
    ...
    VLM2Vec's Stage 1 LoRA path wraps ``encoder.model`` while the vision tower
    remains part of the outer model. The full state alone cannot recreate that
    ...
    nested_model.save_pretrained(output_dir, safe_serialization=True)
    metadata = {
        "format_version": 1,
...
class MMEBTrainer(Trainer):
    ...
    def _save(self, output_dir: Optional[str] = None, state_dict=None):
        ...
        _save_nested_peft_adapter(self.model.encoder, output_dir)
